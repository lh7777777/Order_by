...
int main()
{
	...
	while (1)
	{
		...
		if (a == 1)
		{
			cout << "DN1密文已全部发送完毕,Sort_DN1已全部接收完毕！" << endl;
	...
	for (int i = 1; i <= Label_DN1.size(); i++)
	{
		...
		cin>>label;
			...
			dn1label = fopen("DN1_to_sgxLabel.dat", "wb");//输出Label_DN1[i]
...
void Dn1_update()
{
		...
		dn1sgx = fopen("DN1_to_sgx.dat", "wb");
		...
		while (1)
		{
			...
			{
				//sgx读取文件获得NB个密文
				cout << endl << "********DN 1 已发送给sgx NB个密文，等待sgx处理！" << endl;
				cnt1 = 0;
				break;
			}
		}
...
void getSortDN1()
{
		int cn1 = 0;
		int mc1=1;
		sgxdn1 = fopen("sgx_to_DN1.dat", "rb");
		sgxM1 = fopen("sgx_to_DN1M1.txt", "rb");
		unsigned int* res = (unsigned int*)malloc(17 * (sizeof(int)));
		unsigned char* fres = (unsigned char*)malloc(17 * sizeof(char));
