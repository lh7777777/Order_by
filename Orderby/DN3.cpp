...
map<int, string> Sort_DN3;//部分排序好密文
map<int, int> M3;//每个Label数量
int cnt3 = 0;
map<int, vector<string>> Label_DN3;//按照Label排序好密文
...
int main()
{
	...
	while (1)
	{
		...
		if (a == 1)
		{
			cout << "DN3密文已全部发送完毕,Sort_DN3已全部接收完毕！" << endl;
			break;
		}
	...
	for (int i = 1; i <= Label_DN3.size(); i++)
	{
		...
		cin>>label;
			...
			dn3label = fopen("DN3_to_sgxLabel.dat", "wb");//输出Label_DN3[i]
...
void Dn3_update()
{
		...
		dn3sgx = fopen("DN3_to_sgx.dat", "wb");
		...
		while (1)
		{
			...
			{
				//sgx读取文件获得NB个密文
				cout << endl << "********DN 3 已发送给sgx NB个密文，等待sgx处理！" << endl;
				cnt3 = 0;
				break;
			}
		}
...
void getSortDN3()
{
		int cn3 = 0;
		int mc3=1;
		sgxdn3 = fopen("sgx_to_DN3.dat", "rb");
		sgxM3 = fopen("sgx_to_DN3M3.txt", "rb");
		unsigned int* res = (unsigned int*)malloc(17 * (sizeof(int)));
		unsigned char* fres = (unsigned char*)malloc(17 * sizeof(char));
