...
void Blackbox_update(int dnc);//读取密文并进行部分排序并发送给DN
void Blackbox_orderby();//最终排序

int o = 1;

int main()
{
	ifstream de("c_to_sgx.txt");
	de>>sk>>m;
	de.close();
	cout << "密钥sk：" << sk << "Label数量m:" << m << endl;
	sklen=strlen(sk);
	//对三个DN分别进行部分排序
	Blackbox_update(1);
	Blackbox_update(2);
...
void getDN(int dnc)
{
	...
	{
		int cn1 = 0;
		dn1sgx = fopen("DN1_to_sgx.dat", "rb");
	...
	{
		int cn2 = 0;
		dn2sgx = fopen("DN2_to_sgx.dat", "rb");
	...
	{
		int cn3 = 0;
		dn3sgx = fopen("DN3_to_sgx.dat", "rb");
...
void Blackbox_update(int dnc)
{
	...
	while (1)
	{
		...
		if (dnc == 1)
		{
			...
			sgxdn1 = fopen("sgx_to_DN1.dat", "wb");//输出Sort_DN1
			...
			outfile1.open("sgx_to_DN1M1.txt");
		...
		else if (dnc == 2)
		{
			...
			sgxdn2 = fopen("sgx_to_DN2.dat", "wb");
			...
			outfile2.open("sgx_to_DN2M2.txt");
		...
		{
			//清空输出的文件sgx_to_DN1.dat
			sgxdn3 = fopen("sgx_to_DN3.dat", "wb");
			...
			outfile3.open("sgx_to_DN3M3.txt");
