...
int main()
{
	...
	while (1)
	{
		...
		if (a == 1)
		{
			cout << "DN2密文已全部发送完毕,Sort_DN2已全部接收完毕！" << endl;
	...
	for (int i = 1; i <= Label_DN2.size(); i++)
	{
		...
		cin>>label;
			...
			dn2label = fopen("DN2_to_sgxLabel.dat", "wb");//输出Label_DN2[i]
...
void Dn2_update()
{
		...
		dn2sgx = fopen("DN2_to_sgx.dat", "wb");
		...
		while (1)
		{
			...
			{
				//sgx读取文件获得NB个密文
				cout << endl << "********DN 2 已发送给sgx NB个密文，等待sgx处理！" << endl;
				cnt2 = 0;
				break;
			}
		}
...
void getSortDN2()
{
		int cn2 = 0;
		int mc2=1;
		sgxdn2 = fopen("sgx_to_DN2.dat", "rb");
		sgxM2 = fopen("sgx_to_DN2M2.txt", "rb");
		unsigned int* res = (unsigned int*)malloc(17 * (sizeof(int)));
		unsigned char* fres = (unsigned char*)malloc(17 * sizeof(char));
